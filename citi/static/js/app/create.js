'use strict';
require(['jquery', 'app/form', 'imageUpload', 'jquery.json'], function($, _, ImageUpload) {
    $(document).ready(function() {
        ...
        var addTag = $('.js-add-tag'),
            ...
            func = function() {
                ...
                $this.find('div').focus().on('blur', function() {
                    $(this).html('添加+');
        ...
        next.on('click', function() {
            if (form.data('status') === 1) {
                ...
                var tag = $('#project-tag');
                tag.val() === '' ? tag.val('[]') : tag.val($.toJSON(tag.val().split(',')));
                var data = $('.js-sign-content').children().eq(1).find('input, textarea, select').serialize();
                $.ajax({
                    type: 'post',
                    ...
                    function(data) {
                        ...
                        var errorInInfo = $.map(response, function(v, i) {
        ...
        var first = $('.imageWrap'),
            imageUpload = new ImageUpload.ImageUpload({
                ...
                readTrigger: first.find('.readTrigger'),
            ...
            var newFeedback = $(feedbackTemplate.replace(variable, feedbackCount + 1)).insertBefore(addFeedback).slideDown(),
                imageUpload = new ImageUpload.ImageUpload({
                    ...
                    imageUpload: function () {
                        newFeedback.find('.saveFeedback').prop('disabled', true);
                    },
                    cancel: function () {
                        this.readTrigger.show();
                    },
                    callbacks: {
                        201: function(response) {
                            newFeedback.find('.saveFeedback').prop('disabled', false);
                            newFeedback.find('.imageId').val(response.id);
                        }
                    }
                });
            // newFeedback.data('imageupload', imageUpload);
            // packages.trigger('addFeedback', feedbackCount);
            feedbackCount = feedbackCount + 1;
        })
        addPackages.on('click', function() {
            var newPackages = $(packagesTemplate.replace(variable, packagesCount + 1)).insertBefore(addPackages).slideDown(function () {
                if (packagesCount > 1) {
                    for (var i = 1; i <= feedbackCount; i = i + 1) {
                        newPackages.find('.reward-payback-list').append($(feedbackListTemplate.replace(variable, i)))
                    }
                }
            });
        ...
        $(document).on('click', '.saveFeedback, .savePackages, .panel .close', function() {
            var $this = $(this),
                that = this,
                parent = $this.parents('.panel'),
                index = parent.parent().children('.panel').index(parent) + 1,
                isFeedback = parent.parent().hasClass('reward-info'),
                ...
                isModify = !!parent.data('modify'),
                url = isFeedback ? '/api/crowdfunding/feedback/' : '/api/crowdfunding/package/';
            if (!isClose) {
                if (isFeedback) {
                    data += '&project=' + form.data('id');
                } else {
                    data = data + '&project=' + form.data('id');
                    data = data + '&name=' + parent.find('h4').text();
                    var result = {};
                    data.split('&').forEach(function(v) {
                        var temp = v.split('=');
                        result[temp[0]] = isNaN(parseInt(temp[1])) ? temp[1] : parseInt(temp[1]);
                    })
                    result.feedback = parent.find('.reward-payback-list').data('checkedValue').split(',').map(function(v) {
                        return parseInt(v)
                    });
                    data = $.toJSON(result);
            ...
            $.ajax({
                ...
                function(data) {
                    if (!isClose) {
                        ...
                        $('<span class="tooltip bg-pink">保存成功</span>').css({bottom: 10}).appendTo(parent).hide().fadeIn().delay(2000).fadeOut();
                        $this.hide();
                        $this.siblings().show();
                        if (isFeedback) {
                            packages.trigger('addFeedback', index);
        ...
        $(document).on('click', '.modifyPackages, .modifyFeedback', function () {
            var $this = $(this),
                parent = $this.parents('.panel'),
                isFeedback = parent.parent().hasClass('reward-info'),
                isClose = $this.hasClass('close');
            ...
            if (isFeedback) {
                parent.find('.imageCancel').trigger('click');
            }
            parent.data('modify', true);
            return false;
        })
    })
})
