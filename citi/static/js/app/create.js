...
//                    }
            ...
            addFeedback.on('click', function () {
                var newFeedback = $(feedbackTemplate.replace(variable, feedbackCount))
                    ...
                    imageUpload = new ImageUpload.ImageUpload({
                        ...
                        callbacks: {
                            201: function (response) {
                                newFeedback.find('.imageId')
                                    .val(response.id);
                            }
                        }
                    });
                newFeedback.find('.project')
                    .val(id);
            ...
            packages.on('addFeedback', function (event, text) {
                packages.find('.panel')
                    .each(function () {
                        $(this)
                            .find('.reward-payback-list')
                            .append($(feedbackListTemplate.replace(variable, text)));
                    })
            })
            $(document)
                .on('close', '.panel .close', function (event) {
                    var $this = $(event.target),
                        ...
                        isFeedback = parent.parent()
                            .hasClass('reward-info');
                    ...
                    if (isFeedback) {
                        packages.find('.reward-payback-list')
                            .each(function () {
                                var $$this = $(this),
                                    inputList;
                                $$this.children()
                                ...
                                $$this.data('checkedValue') && $.each($$this.data('checkedValue')
                                    .split(','), function (i, v) {
                                    if (i > index) {
                                        inputlist.eq(i - 1)
                                            .prop('checked', true);
            ...
            $(document)
                .on('click', '.reward-payback-list  input', function (event) {
                    var $this = $(this),
                        ul = $this.parents('.reward-payback-list'),
                        checkedValue = [];
                    ul.find('input:checked')
                        .each(function () {
                            checkedValue.push($(this)
                                .val());
                        })
                    ul.data('checkedValue', checkedValue.join(','))
                })
            $(document)
                .on('click', '.saveFeedback, .savePackages, .panel .close', function () {
                    var $this = $(this),
                        that = this,
                        parent = $this.parents('.panel'),
                    ...
                    if (!isClose) {
                        if (isFeedback) {
                            ...
                            data = data + '&feedback=[' + parent.find('.reward-payback-list')
                                .data('checkedValue')
                                .split(',')
